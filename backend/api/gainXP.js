...
export default async function handler(req, res) {
  if (req.method === "POST") {
    ...
    if (fetchError) return res.status(500).json({ message: "Error fetching user", fetchError });

    const newXP = (userData.xp || 0) + xpGained;
    const newLevel = Math.floor(newXP / 100); // 100 XP per level
    ...
    if (updateError) return res.status(500).json({ message: "Error updating XP", updateError });

    res.status(200).json({ message: "XP updated", xp: newXP, level: newLevel });
  } else {
    res.setHeader("Allow", ["POST"]);
    res.status(405).json({ message: `Method ${req.method} not allowed` });
  }
}
3️⃣ getUserStats.js — fetch user stats
js
Copy code
// api/getUserStats.js
const { createClient } = require('@supabase/supabase-js');
